...
export function updateInfoForm(form, {
    info,
    containers
}) {
    if (containers) {
...
export function removeChildren(node, direction = 'fromFirst', count = Infinity) {
    const { children } = node;
    let i = 0;
...
export function htmlBrandNewChildren(node, children) {
    removeChildren(node);
...
export function getBrowserContainers() {
    return browserContainers;
}

export async function initBrowserContainers() {
